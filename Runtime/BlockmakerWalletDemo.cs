...
public sealed class BlockmakerWalletDemo : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        status = new Label("Preparing wallets…"); status.style.color = new Color(0.94f, 0.96f, 1f); root.Add(status);
        picture = new Image(); picture.style.width = picture.style.height = 128; picture.style.display = DisplayStyle.None; root.Add(picture);
    ...
    private void Open()
    {
        ...
        view.OpenAccount(result => {
            ...
            connect.SetEnabled(!ready); profile.SetEnabled(ready); logout.SetEnabled(ready);
    ...
    private IEnumerator LoadPicture(string source, string wallet, int read)
    {
        ...
        using (var request = UnityWebRequestTexture.GetTexture(source)) {
            ...
            ClearPicture(); pictureTexture = texture; picture.image = texture; picture.style.display = DisplayStyle.Flex;
        }
    }
